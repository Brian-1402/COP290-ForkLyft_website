...
def test_add_to_cart1(client, auth, app):
    ...
    response = client.get(
        "/add_to_cart?item_id=1413414101&restaurant_id=14134141", follow_redirects=True
    )
    response = client.get(
        "/add_to_cart?item_id=1413414101&restaurant_id=14134141", follow_redirects=True
    )
    response = client.get(
        "/add_to_cart?item_id=1413414102&restaurant_id=14134141", follow_redirects=True
    )
    response = client.get(
        "/add_to_cart?item_id=1413414102&restaurant_id=14134141", follow_redirects=True
    )
...
def test_pay(client, auth, app):
    ...
    response = client.post(
        "/user/pay",
        data={"flexRadioDefault": "test_other_address"},
    ...
    with app.app_context():
        ...
        with db.connect() as conn:  # testing if the order_status field is updated to "pending"
            cart = conn.execute(
                text(
                    'SELECT * FROM orders WHERE user_id = :id AND order_id = :order_id AND order_status = "pending" ORDER BY order_item_id'
                ),
                {"id": 121098561, "order_id": 1},
            ).all()
            assert len(cart) == 2
...
def test_cart_buttons(client, auth, app):
    ...
    auth.login_user()
    response = client.get(
        "/increase?item_id=1413414102&quantity=1", follow_redirects=True
    )
    assert b"quantity: 2" in response.data
    response = client.get(
        "/decrease?item_id=1413414101&quantity=3", follow_redirects=True
    )
    assert b"quantity: 2" in response.data
    response = client.get(
        "/decrease?item_id=1413414101&quantity=2", follow_redirects=True
    )
    assert b"quantity: 1" in response.data
    response = client.get(
        "/decrease?item_id=1413414101&quantity=1", follow_redirects=True
    )
...
def test_delete_order_pending(client, auth, app):
    ...
    with app.app_context():
        ...
        with db.connect() as conn:
            cart = conn.execute(
                text(
                    'SELECT * FROM orders WHERE order_id = :id AND order_status = "done" ORDER BY order_item_id'
                ),
                {"id": 1},
            ).all()
            assert len(cart) == 2
