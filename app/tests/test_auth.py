...
def test_register_user(client, app):
    ...
    wrong_response1 = client.post("/signup", data={"username": "tester_login", "password": "tester_signup", "mail":"tester_signup@tests.com","contact":"1234567890","name":"tester_signup"}, follow_redirects=True)
...
def test_register_restaurant(client, app):
    ...
    with app.app_context():
        with get_db().connect() as conn:
            assert conn.execute(text("SELECT * FROM restaurants WHERE restaurant_username = 'tester_signup_res1'")).fetchone() is not None
