...
def test_restaurant_remove_item(client, auth, app):
    ...
    with app.app_context():
        db = get_db()
        with db.connect() as conn:
            item = conn.execute(text('SELECT * FROM menus WHERE restaurant_id = :id AND menu_id = :item_id ORDER BY menu_id'),{'id':14134141, "item_id":1413414103}).all()
            assert len(item)==0
            full_menu = conn.execute(text('SELECT * FROM menus WHERE restaurant_id = :id ORDER BY menu_id'),{'id':14134141}).all()
